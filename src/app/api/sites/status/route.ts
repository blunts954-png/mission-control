...
export async function GET() {
  try {
    ...
    const [betterStackData, ...siteChecks] = await Promise.all([
      ...
      ...SITES_CONFIG.map(site => checkSiteStatus(site.url))
    ])

    // Fetch Netlify deploy status for all sites
    const netlifyStatuses = await Promise.all(
      SITES_CONFIG.map(site => site.netlifyId ? fetchNetlifyStatus(site.netlifyId) : Promise.resolve(null))
    )

    // Build response
    const sites: SiteStatus[] = SITES_CONFIG.map((site, index) => {
      ...
      return {
        ...
        securityIssues: 0, // Will be updated when security scanning is implemented
